...
function debug(...info) {
    if (options.debug) {
        return console.log(info.join(" "))
    }
}
function install(dir = options.directory || "./", count=options.c, nogif = options["no-gif"] || false) {
    function download(directory, url) {
        imgd.image({ url: url, dest: directory }).then(({ filename }) => {
            debug(c.greenBright("[INFO] -"), `${filename} downloaded!`);
        }).catch(e => { return debug(c.redBright("[ERROR] -"), `Can't download because of an error: ${e}`); });
    };
    try {
        fs.readdirSync(dir);
    } catch {
        debug(c.redBright("I cannot find that folder. Please try again"));
        throw new Error(`I cannot find that folder "${dir}". Please try again`)
    }
    debug(c.yellowBright(`Getting memes from URL...`));
    fetch(`https://meme-api.herokuapp.com/gimme/${count || 10}`).then(m => m.json()).then(m => {
        debug(c.yellowBright(`Downloading ${!m.count ? 1 : m.count} meme${!m.count ? '' : m.count > 1 ? 's' : ''}...`));
        ...
        if (memes instanceof Array) {
            for (const meme of memes) {
                ...
                if ((typeof nogif != "undefined" && nogif) && filememe[filememe.length - 1] == "gif") {
                    debug(c.cyanBright("[SKIPPED] -"), "Skipped because the file is a GIF and it's not allowed by an option \"--no-gif\"");
                    continue;
                } else {
                    download(dir, meme.url);
                }
            }
        } else {
        ...
        return debug(c.redBright("[ERROR] -"), `Can't get an URL because of an error: ${e}`);
    });
}
module.exports = { install, debug, welcome }
